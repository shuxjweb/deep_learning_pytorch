...
def update_D(X, Z, net_D, net_G, loss, trainer_D):    # [8, 2], [8, 2]
    ...
    real_Y = net_D(X.float())   # [8, 1]
    fake_X = net_G(Z)           # [8, 2]
    fake_Y = net_D(fake_X)      # [8, 1]
...
def train(net_D, net_G, data_iter, num_epochs, lr_D, lr_G, latent_dim, data):
    ...
    d_loss = 0
    g_loss = 0
    for epoch in range(1, num_epochs + 1):        # 20
        d_loss_sum = 0
        g_loss_sum = 0
        batch = 0
        for X in data_iter:       # [8, 2]
            batch += 1
            X = Variable(X)
            batch_size = X.shape[0]
            Z = Variable(Tensor(np.random.normal(0, 1, (batch_size, latent_dim))))      # [8, 2]
            trainer_D.zero_grad()
