...
def train_ch3(net, train_iter, test_iter, loss, num_epochs, batch_size,
    ...
    for epoch in range(num_epochs):      # 5
        ...
        for X, y in train_iter:       # [256, 1, 28, 28], [256,]
            y_hat = net(X)            # [256, 10]
            l = loss(y_hat, y).sum()
...
def load_data_fashion_mnist(batch_size, resize=None, root='~/Datasets/FashionMNIST'):
    ...
    mnist_train = torchvision.datasets.FashionMNIST(root=root, train=True, download=True, transform=transform)     # [60000, 28, 28]
    mnist_test = torchvision.datasets.FashionMNIST(root=root, train=False, download=True, transform=transform)     # [10000, 28, 28]
    if sys.platform.startswith('win'):
        num_workers = 0  # 0表示不用额外的进程来加速读取数据
    else:
        num_workers = 4
...
def load_data_jay_lyrics():
    """加载周杰伦歌词数据集"""
    with zipfile.ZipFile('input/jaychou_lyrics.txt.zip') as zin:
